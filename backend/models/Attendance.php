...
<?php
// Attendance model: handles all attendance-related DB operations
...
class Attendance
{
    ...
    public static function delete($id) {
        if (!$id || !is_numeric($id)) return false; // Validate ID
        $db = Database::getConnection(); // Get DB connection
        $stmt = $db->prepare("DELETE FROM attendance WHERE id = ?");
        return $stmt->execute([$id]); // Execute delete
    }

    // Bulk delete attendance records by array of IDs
    public static function deleteMany($ids) {
        if (!is_array($ids) || empty($ids)) return false; // Validate input
        $db = Database::getConnection();
        // Only allow numeric IDs
        $ids = array_filter($ids, 'is_numeric');
        if (empty($ids)) return false;
        $in = str_repeat('?,', count($ids) - 1) . '?'; // Prepare placeholders
        $stmt = $db->prepare("DELETE FROM attendance WHERE id IN ($in)");
        return $stmt->execute($ids); // Execute bulk delete
    }

    // Record a check-in for a user
    public static function checkIn($userId, $type = 'checkin') {
        require_once __DIR__ . '/../services/GroupService.php'; // Group logic
        require_once __DIR__ . '/Department.php'; // Department logic
        $db = Database::getConnection();

        // Block if not a valid group day
        if (!\GroupService::isTodayAllowed($userId)) {
            return ['success' => false, 'message' => 'Check-in not allowed today (not your group day).'];
        }

        // Get department check-in time limit
        ...
        $checkInLimit = $department['check_in_limit'] ?? '09:00:00'; // Default limit
        $now = date('H:i:s');
        if ($now > $checkInLimit) {
    ...
    public static function checkOut($userId) {
        ...
        $checkOutLimit = $department['check_out_limit'] ?? '17:00:00'; // Default limit
        $now = date('H:i:s');
    ...
    public static function getToday($userId) {
        ...
        $stmt->execute([$userId]);
        return $stmt->fetch(PDO::FETCH_ASSOC); // Return record or null
    }
    ...
    // Get all attendance logs (admin use), with filters
    ...
    // Get attendance records by date (admin filter)
    public static function getByDate($date) {
        ...
        $stmt->execute([$date]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC); // Return all records for date
    }
}